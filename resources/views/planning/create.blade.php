        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <div class="text-sm text-gray-600">
                    ...
                    <span class="text-gray-400">/</span>
                    <span class="text-gray-600">Nouvel événement</span>
                </div>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-sm">
                <div class="p-6">
                    <form method="POST" action="{{ route('planning.store', $group) }}" class="space-y-4">
                        @csrf
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <select id="type" name="type" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400">
                                    @foreach(['event' => 'Événement', 'repetition' => 'Répétition', 'rendezvous' => 'Rendez-vous', 'task' => 'Tâche'] as $k => $label)
                                        <option value="{{ $k }}" @selected(old('type', 'event') === $k)>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('type')
                            ...
                            <div>
                                ...
                                <input id="title" name="title" type="text" value="{{ old('title') }}" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400" />
                                @error('title')
                                    <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <input id="location" name="location" type="text" value="{{ old('location') }}" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400" />
                            @error('location')
                                <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="starts_at" name="starts_at" type="datetime-local" value="{{ old('starts_at') }}" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400" />
                                @error('starts_at')
                                    <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="ends_at" name="ends_at" type="datetime-local" value="{{ old('ends_at') }}" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400" />
                                @error('ends_at')
                                    <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="5" class="mt-2 block w-full rounded-2xl border-gray-200 bg-white focus:border-gray-400 focus:ring-gray-400">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <a href="{{ route('planning.index', $group) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-semibold text-gray-900 hover:bg-gray-50 transition">
                                Annuler
                            </a>
